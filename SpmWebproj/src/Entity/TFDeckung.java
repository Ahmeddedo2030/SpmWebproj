...
public class TFDeckung {
	public Map <String, Float> findMaximum(Instances daten, int index) throws Exception {
    	...
    	Map<String, Integer> b=a.ReadPrice(path);//"C:/Users/小彭子hhh/Desktop/java程序/SpmWebproj/SpmWebproj/WebContent/WEB-INF/upload/deckungsspanne.txt"
		...
		for(int j =11;j<daten.numAttributes();j++) {
	            int tmp = 0;
	            for (int i = 0; i < daten.size(); i++) {//�õ�����alleDaten.numInstances();
		...
		Map <String, Float> result= new LinkedHashMap<>();//hashmap��treeMap������ģ�ֻ��LinkedMap�������
		/*float f = 0;
		float flop = 100000;
		for(Map.Entry<Integer, Float> entry: map1.entrySet() ) {//f���������ֵ��flop��������Сֵ
			if(entry.getValue()>f)
				f=entry.getValue();
		...
		for(Map.Entry<Integer, Float> entry: map1.entrySet() ) {
			...
			if(entry.getValue()==flop)
				//System.out.println("��С����Ʒ "+ daten.attribute(entry.getKey()).name()+entry.getValue());
				result.put(daten.attribute(entry.getKey()).name(),entry.getValue());
		}*/
		   ArrayList<Map.Entry<String, Float>> list = new ArrayList<Map.Entry<String, Float>>(map1.entrySet());
	        Collections.sort(list, new Comparator<Map.Entry<String, Float>>() {//��Map����valueֵ����
